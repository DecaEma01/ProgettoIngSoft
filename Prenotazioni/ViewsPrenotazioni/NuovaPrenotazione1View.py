...
class NuovaPrenotazione1View(object):
    ...
    def visualizzaListaPazienti(self):
        ...
        for paziente in self.pazienti:
            item = QStandardItem()
            etichetta = f" {paziente.nome} {paziente.cognome} - {paziente.codicePaziente}"
    ...
    def selezionaPaziente(self, form, app):
        try:
            selected = self.listView.selectedIndexes()[0].data()
            codice = int(selected[0])

            paziente = ElencoPazientiController.ricercaPazienteC(ElencoPazientiController, codicePaziente=codice)[
                codice]
            ...
            QtWidgets.QMessageBox.critical(form, 'Errore', 'Selezionare un paziente specifico! ',
